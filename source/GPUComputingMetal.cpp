...
float* GPUComputing::get_potentials_from_tubes_with_integral(const float* tubes, int tubes_size, const float* points, int points_size, float R, float G) {
    // create autorelease pool
    auto pool = NS::TransferPtr(NS::AutoreleasePool::alloc()->init());
    static NS::String* source = nullptr;
    if(source == nullptr) {
        ...
        // read shader
        std::ifstream file;
        file.open("../shaders/get_potentials_from_tubes.metal", std::ios::in);
        std::stringstream ss;
        ss << file.rdbuf();
    ...
    static auto function = NS::TransferPtr(library->newFunction(
            NS::String::string("get_potentials_from_tubes", NS::ASCIIStringEncoding)
    ));
    NS::Error* e_2 = nullptr;
    static auto computePipelineState = NS::TransferPtr(device->newComputePipelineState(function.get(), &e_2));
    if(e_2) std::cout << e_2->localizedDescription()->utf8String();
    ...
    auto G_buffer = NS::TransferPtr(device->newBuffer(&G, sizeof(float), MTL::ResourceStorageModeShared));
    auto R_buffer = NS::TransferPtr(device->newBuffer(&R, sizeof(float), MTL::ResourceStorageModeShared));
    auto output_buffer = NS::TransferPtr(device->newBuffer(sizeof(float) * points_size, MTL::ResourceStorageModeShared));
    ...
    auto output = (float*)malloc(points_size * sizeof(float));
    if(output == nullptr) {
        std::cout << "failed to allocate memory for gpu output array" << std::endl;
        if(output_buffer->contents() == nullptr) std::cout << "even gpu buffer output is null" << std::endl;
    }

    memcpy(output, output_buffer->contents(), points_size * sizeof(float));

    return output;
}

float* GPUComputing::get_potential_from_tubes_with_integral(const float* tubes, int tubes_size, float* p, float R, float G) {
    // create autorelease pool
    auto pool = NS::TransferPtr(NS::AutoreleasePool::alloc()->init());
    static NS::String* source = nullptr;
    if(source == nullptr) {
        ...
        // read shader
        std::ifstream file;
        file.open("../shaders/get_potential_from_tubes.metal", std::ios::in);
        std::stringstream ss;
        ss << file.rdbuf();
    ...
    static auto function = NS::TransferPtr(library->newFunction(
            NS::String::string("get_potential_from_tubes", NS::ASCIIStringEncoding)
    ));
    NS::Error* e_2 = nullptr;
    static auto computePipelineState = NS::TransferPtr(device->newComputePipelineState(function.get(), &e_2));
    if(e_2) std::cout << e_2->localizedDescription()->utf8String();
    ...
    auto G_buffer = NS::TransferPtr(device->newBuffer(&G, sizeof(float), MTL::ResourceStorageModeShared));
    auto R_buffer = NS::TransferPtr(device->newBuffer(&R, sizeof(float), MTL::ResourceStorageModeShared));
    auto output_buffer = NS::TransferPtr(device->newBuffer(sizeof(float)*tubes_size, MTL::ResourceStorageModeShared));
    ...
    auto output = (float*)malloc(tubes_size * sizeof(float));
    memcpy(output, output_buffer->contents(), tubes_size * sizeof(float));
